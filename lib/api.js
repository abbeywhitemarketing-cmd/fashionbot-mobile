...
async function getRecentOutfits() {
  try {
    ...
    const dates = [tomorrowStr, todayStr, ...getPastDates(14)];
...
export async function fetchShopProducts(items, outfitDate, palette = [], challenge = "", styleKeywords = "") {
  const response = await fetch(`${BASE_URL}/shop/products`, {
    ...
    body: JSON.stringify({ items, outfit_date: outfitDate, palette, challenge, style_keywords: styleKeywords }),
  });
  if (!response.ok) throw new Error(`Shop products failed: ${response.status}`);
  return response.json();
}

export async function warmOutfit(date, preferences, rcUserId) {
  const recentOutfits = await getRecentOutfits();
  try {
    await fetch(`${BASE_URL}/outfit/warm`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-app-secret": APP_SECRET,
...
export async function fetchOutfit(date, preferences, calendarEvents = [], rcUserId, forceRefresh = false) {
  const recentOutfits = await getRecentOutfits();
